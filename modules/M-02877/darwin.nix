      ...
      launchd.user.agents = {
        ...
        phoenix = {
          serviceConfig = {
            ...
            ProgramArguments = [
              "/bin/sh" "-c"
              "C=/opt/homebrew/bin/container; $C network create proxy-chain 2>/dev/null; $C stop phoenix 2>/dev/null; $C rm phoenix 2>/dev/null; $C image pull docker.io/arizephoenix/phoenix:latest && exec $C run --rm --name phoenix --network proxy-chain -p 6006:6006 -e PHOENIX_DEFAULT_RETENTION_POLICY_DAYS=30 -e PHOENIX_PROJECT_NAME=local-model-proxy docker.io/arizephoenix/phoenix:latest"
            ];
            RunAtLoad = true;
            KeepAlive = true;
          ...
          serviceConfig = {
            ...
            ProgramArguments = [
              ...
              ''
                ...
                for i in $(seq 1 30); do
                  /usr/bin/curl -sf -m 2 http://localhost:8787/health >/dev/null 2>&1 && break
                  sleep 2
                done
                ...
                exec $C run --rm --name local-model-proxy --network proxy-chain -p 8788:8788 \
